"""OOS/negative generation & polluted splits (+ debug helper)."""
import re
import random
import numpy as np
import pandas as pd
...
def generate_fallback_negatives_en(
    n: int = 100,
    seed: int = 42,
    avoid_like_df: pd.DataFrame | None = None,
    max_trials_per_item: int = 10,
    gibberish_ratio: float = 0.3
) -> pd.DataFrame:
    """Generate English NEG/OOS sentences; label is intent="__NEG__"."""
    ...
    def make_sentence(i: int) -> str:
        if rng.random() < (1.0 - gibberish_ratio):
            topic = rng.choice(topics_en)
            tpl = rng.choice(question_templates + command_templates)
            return tpl.format(topic=topic)
        else:
            tpl = rng.choice(gibberish_templates)
...
def make_polluted_test(
    ...
    """Return polluted test with OOS/NEG."""
...
def make_polluted_test_debug(
    ...
    """Same as make_polluted_test, but prints steps for debugging."""
    rng = np.random.RandomState(seed)
    n_oos = int(len(test_df) * ratio)
