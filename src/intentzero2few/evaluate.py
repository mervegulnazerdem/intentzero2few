...
"""
Evaluation helpers (super-intent + OOS)
Produces:
  * classification_report (dict)
  * confusion_matrix (raw and row-normalized)
  * overall accuracy and macro-F1
Notes:
  * y_true is built by mapping fine-grained intents to their super-intent, or OOS.
  * y_pred is obtained from zs_model similarities with threshold τ.
  * Only comments/docstrings added here."""
...
def _true_supers(df: pd.DataFrame, mapping: Dict[str,str], intent_col: str = "intent", is_oos_col: str = "is_oos") -> List[str]:
    ...
    """Map fine-grained labels → {super-intent | OOS}."""
...
def evaluate_superintent(
    ...
    """Return a compact report dict for the given split and threshold τ."""
    ...
    return {
        ...
        "confusion_matrix_normalized": cmn.tolist(),
        "accuracy": float(acc),
