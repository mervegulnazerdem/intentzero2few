...
def setfit_fewshot(model_name, train_df, val_df, le, num_epochs=3, batch_size=16):
    from setfit import SetFitModel, SetFitTrainer
    m = SetFitModel.from_pretrained(model_name)
    t = SetFitTrainer(model=m, train_dataset=train_df, eval_dataset=val_df, metric="accuracy", batch_size=batch_size, num_epochs=num_epochs)
    t.train()
    print(t.evaluate())
    return m
